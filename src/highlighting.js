...
export function setupHighlighting(sourceWrapper, matches) {
  ...
  function hideInfoPopup() {
    ...
    try { if (currentPopupAnchor?.data?.group) toggleGroupHover(currentPopupAnchor.data.group, false); } catch {}
  ...
  function showInfoPopup(target, data) {
    if (!infoPopup) return;
    try { console.log('[HL][POPUP] showInfoPopup kind=%s label=%s field=%s line=%s raw=%o group=%s', data?.kind || 'value', data?.label || data?.field, data?.field, data?.line, data?.raw, data?.group); } catch {}
    // Build content
  ...
  try { if (data && data.group) toggleGroupHover(data.group, true, data.variant || null); } catch {}
  }
  ...
  function processBadgeClick(el, scopeWrapper) {
    ...
    try { console.log('[BADGE][CLICK] group=%s variant=%s hasId=%s [scope=%s]', group, variant, String(el.hasAttribute('data-hl-id')), scopeWrapper === sourceWrapper ? 'primary' : 'global'); } catch {}
    if (group) {
      try {
        ...
        if (directId && Array.isArray(scopeWrapper.__hlMap)) {
          ...
          if (data) {
            try { console.log('[BADGE][CLICK][DIRECT] id=%s -> kind=%s label=%s field=%s line=%s raw=%o', directId, data?.kind || 'value', data?.label || data?.field, data?.field, data?.line, data?.raw); } catch {}
        ...
        try { console.log('[BADGE][CLICK][FALLBACK] candidates=%d group=%s', candidates.length, group); } catch {}
        ...
        if (targetEl && bestData) {
          try { console.log('[BADGE][CLICK][CHOSEN] id=%s kind=%s label=%s field=%s line=%s raw=%o', targetEl.getAttribute('data-hl-id'), bestData?.kind || 'value', bestData?.label || bestData?.field, bestData?.field, bestData?.line, bestData?.raw); } catch {}
          try { toggleGroupHover(group, true, variant); } catch {}
          showInfoPopup(targetEl, { ...bestData, variant });
          return true;
        }
        try { console.warn('[BADGE][CLICK][NO-CANDIDATE] group=%s', group); } catch {}
        toggleGroupHover(group, true, variant);
        currentPopupAnchor = { el, data: { group, variant } };
        return false;
      } catch {}
  ...
  sourceWrapper.addEventListener('click', (e) => {
    ...
    if (idAttr && sourceWrapper && Array.isArray(sourceWrapper.__hlMap)) {
      ...
      const data = sourceWrapper.__hlMap[idx];
      try { console.log('[HL][CLICK] id=%s -> idx=%d, kind=%s, label=%s, field=%s', idAttr, idx, data?.kind || 'value', data?.label || data?.field, data?.field); } catch {}
      if (data) { showInfoPopup(el, data); return; }
    }
  });
  ...
  const toggleGroupHover = (group, on, variantOverride = null) => {
    ...
    try {
      ...
      try { console.log('[HL][GROUP] %s group=%s, nodes=%d', on ? 'enter' : 'leave', group, nodes.length); } catch {}
      nodes.forEach(n => {
        ...
        if (on && variantOverride) { try { n.setAttribute('data-hl-variant', String(variantOverride)); } catch {} }
      });
      ...
      badges.forEach(b => {
        ...
        if (on && variantOverride) { try { b.setAttribute('data-hl-variant', String(variantOverride)); } catch {} }
      });
    } catch {}
  };
